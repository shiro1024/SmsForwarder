...
class ActionWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        if (!taskConditionsJson.isNullOrEmpty()) {
            val conditionList =
                Gson().fromJson(taskConditionsJson, Array<TaskSetting>::class.java).toMutableList()
        ...
        val actionList =
            Gson().fromJson(taskActionsJson, Array<TaskSetting>::class.java).toMutableList()
            ...
            try {
                when (action.type) {
                    TASK_ACTION_SENDSMS -> {
                        ...
                        val msg = if (ActivityCompat.checkSelfPermission(
                                App.context,
                            ...
                            PhoneUtils.sendSms(
                                mSubscriptionId,
                                smsSetting.phoneNumbers,
                                smsSetting.msgContent
                            )
                        }
                        if (msg == null || msg == "") {
                            successNum++
                            writeLog(
                                String.format(
                                    getString(R.string.successful_execution),
                                    smsSetting.description
                                ), "SUCCESS"
                            )
                        } else {
                            writeLog(msg, "ERROR")
                    ...
                    TASK_ACTION_CLEANER -> {
                        val cleanerSetting =
                            Gson().fromJson(action.setting, CleanerSetting::class.java)
                    ...
                    TASK_ACTION_SETTINGS -> {
                        val settingsSetting =
                            Gson().fromJson(action.setting, SettingsSetting::class.java)
                        ...
                        SettingUtils.locationPowerRequirement =
                            settingsSetting.locationPowerRequirement
                        ...
                        SettingUtils.enableLoadSystemAppList =
                            settingsSetting.enableLoadSystemAppList
                        SettingUtils.cancelExtraAppNotify = settingsSetting.cancelExtraAppNotify
                        SettingUtils.duplicateMessagesLimits =
                            settingsSetting.duplicateMessagesLimits
                        ...
                        writeLog(
                            String.format(
                                getString(R.string.successful_execution),
                                settingsSetting.description
                            ), "SUCCESS"
                        )
                    }

                    TASK_ACTION_HTTPSERVER -> {
                        val httpServerSetting =
                            Gson().fromJson(action.setting, HttpServerSetting::class.java)
                        ...
                        HttpServerUtils.enableApiContactQuery =
                            httpServerSetting.enableApiContactQuery
                        ...
                        HttpServerUtils.enableApiBatteryQuery =
                            httpServerSetting.enableApiBatteryQuery
                    ...
                    TASK_ACTION_SENDER -> {
                        val senderSetting =
                            Gson().fromJson(action.setting, SenderSetting::class.java)
                        ...
                        writeLog(
                            String.format(
                                getString(R.string.successful_execution),
                                senderSetting.description
                            ), "SUCCESS"
                        )
                    }

                    TASK_ACTION_TASK -> {
                        val taskActionSetting =
                            Gson().fromJson(action.setting, TaskActionSetting::class.java)
                    ...
                    TASK_ACTION_RESEND -> {
                        val resendSetting =
                            Gson().fromJson(action.setting, ResendSetting::class.java)
                        if (resendSetting == null) {
                            writeLog("resendSetting is null")
                            continue
                        }

                        val logsList = Core.logs.getIdsByTimeAndStatus(
                            resendSetting.hours,
                            resendSetting.statusList
                        )
