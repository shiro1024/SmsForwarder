...
@Page(name = "主动控制·客户端")
class ClientFragment : BaseFragment<FragmentClientBinding?>(), View.OnClickListener,
    ...
    override fun initTitle(): TitleBar? {
        ...
        if (SettingUtils.enablePureClientMode) {
            titleBar.setTitle(R.string.app_name).setSubTitle(getString(R.string.menu_client))
                .disableLeftView()
    ...
    override fun initListeners() {
        ...
        binding!!.etServerAddress.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                HttpServerUtils.serverAddress =
                    binding!!.etServerAddress.text.toString().trim().trimEnd('/')
            }
        })
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.btn_server_history -> {
                ...
                MaterialDialog.Builder(requireContext()).title(R.string.server_history)
                    ...
                    .itemsCallbackSingleChoice(0) { _: MaterialDialog?, _: View?, _: Int, text: CharSequence ->
                        ...
                        val matches =
                            Regex("【(.*)】(.*)", RegexOption.IGNORE_CASE).findAll(text).toList()
                        ...
                        if (!TextUtils.isEmpty(signKey)) {
                            val keyMatches =
                                Regex("(.*)##(.*)", RegexOption.IGNORE_CASE).findAll(signKey)
                                    .toList().flatMap(MatchResult::groupValues)
                    ...
                    .neutralText(R.string.clear_history).neutralColor(getColors(R.color.red))
    ...
    override fun onItemClick(itemView: View, item: PageInfo, position: Int) {
        try {
            ...
            if (serverConfig != null && ((item.name == getString(R.string.api_sms_send) && !serverConfig!!.enableApiSmsSend) || (item.name == getString(
                    R.string.api_sms_query
                ) && !serverConfig!!.enableApiSmsQuery) || (item.name == getString(R.string.api_call_query) && !serverConfig!!.enableApiCallQuery) || (item.name == getString(
                    R.string.api_contact_query
                ) && !serverConfig!!.enableApiContactQuery) || (item.name == getString(R.string.api_contact_add) && !serverConfig!!.enableApiContactAdd) || (item.name == getString(
                    R.string.api_battery_query
                ) && !serverConfig!!.enableApiBatteryQuery) || (item.name == getString(R.string.api_wol) && !serverConfig!!.enableApiWol) || (item.name == getString(
                    R.string.api_location
                ) && !serverConfig!!.enableApiLocation))
            ) {
                XToastUtils.error(getString(R.string.disabled_on_the_server))
                return
            }
            @Suppress("UNCHECKED_CAST") PageOption.to(Class.forName(item.classPath) as Class<XPageFragment>)
    ...
    private fun queryConfig(needToast: Boolean) {
        ...
        if (HttpServerUtils.clientSafetyMeasures != 0 && TextUtils.isEmpty(clientSignKey)) {
            if (needToast) XToastUtils.error("请输入签名密钥/RSA公钥")
            return
        }
        ...
        postRequest.execute(object : SimpleCallBack<String>() {
            ...
            override fun onSuccess(response: String) {
                ...
                try {
                    ...
                    val resp: BaseResponse<ConfigData> = Gson().fromJson(
                        json,
                        object : TypeToken<BaseResponse<ConfigData>>() {}.type
                    )
                    if (resp.code == 200) {
                        ...
                        //添加到历史记录
                        val key =
                            "【${serverConfig?.extraDeviceMark}】${HttpServerUtils.serverAddress}"
                        if (TextUtils.isEmpty(HttpServerUtils.clientSignKey)) {
                            serverHistory[key] =
                                "SMSFORWARDER##" + HttpServerUtils.clientSafetyMeasures.toString()
                        } else {
                            serverHistory[key] =
                                HttpServerUtils.clientSignKey + "##" + HttpServerUtils.clientSafetyMeasures.toString()
                        }
